...
**notes**
---------
Next week Chapter 15 in Efron and Tibshirani, and read a paper that someone will put on GitHub 
We will transition to Permutation methods and more primary lit.

Classic example is using bootstrap to estimate standard errors. 1) select N independent bootstraps, 2) calculate x statistic, 3) estimated the standard error of the sample statistic x, look at the full distribution of the test statistic from the bootstrapped data.

Usually bootstrap to same number of data points as the original dataset, to perserve a similar amount of variation in the data.
...
ggplot(lut, aes(period, level)) + geom_line() + theme_classic() + 
  geom_hline(yintercept = mean(level), col="cadetblue")
```
