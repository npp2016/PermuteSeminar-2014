...
We will assume that these data follow a first order autoregressive scheme.  First, we define the centered measurements $z_{t}=y_{t}-\mu$, where all $z_{t}$ have expectation $0$.
...
```{r}
lhz <- lh$level - mean(lh$level)
z.t <- as.data.frame(lh$level - mean(lh$level))
lh$z = lhz #defining the new z(t) as y(t) - mean(y) and adding it to the existing data frame
ggplot(lh, aes(period, z)) + geom_line(col="black") + theme_classic() #plot z
```

We expect that $RSE(b)$ has expectation $E(RSE(b))$ and is minimized when $b = \beta$.  We can calculate $RSE(b)$ as a function of $b$, and choose the value that minimizes this function to serve as our estimate of $\beta$.  Remember that $\beta$ varies between $-1$ and $1$.
...
*This is from the book, not from class examples*
