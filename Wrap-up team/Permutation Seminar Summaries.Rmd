...
```{r,warning=F,message=FALSE}
 ...
 avian.ppp=ppp(avian$lon,avian$lat,poly=list(x=rev(avianBoundary[,1]),y=rev(avianBoundary[,2])))
```

```{r,echo=FALSE}
p1=ggplot(as.data.frame(avianBoundary),aes(x=V1,y=V2))+geom_polygon(alpha=0.3)+ theme_bw() + geom_point(data=avian,aes(x=lon,y=lat)) + labs(x="",y="",title="Avian Island (Southern Elephant Seals)")
```


```{r,warning=F,message=FALSE}
pauletBoundary=convex.hull(as.matrix(paulet))$rescoords
paulet.ppp=ppp(paulet$lon,paulet$lat,poly=list(x=rev(pauletBoundary[,1]),y=rev(pauletBoundary[,2])))
```

```{r, fig.width=17,echo=FALSE}
p2=ggplot(as.data.frame(pauletBoundary),aes(x=V1,y=V2))+geom_polygon(alpha=0.3)+ theme_bw() + geom_point(data=paulet,aes(x=lon,y=lat)) + labs(x="",y="",title="Paulet (Antarctic Fur Seals)")
...
avianSub.ppp=ppp(avianSub$lon,avianSub$lat,poly=list(x=rev(avianSubBoundary[,1]),y=rev(avianSubBoundary[,2])))
```
```{r,warning=FALSE}
...
pauletSub.ppp=ppp(pauletSub$lon,pauletSub$lat,poly=list(x=rev(pauletSubBoundary[,1]),y=rev(pauletSubBoundary[,2])))
```

```{r,echo=FALSE,fig.width=17}
p1=ggplot(pauletSubBoundary,aes(x=V1,y=V2))+geom_polygon(alpha=0.3)+ theme_bw() + geom_point(data=pauletSub,aes(x=lon,y=lat))+ labs(x="",y="",title="Avian Island (Southern Elephant Seals)")
p2=ggplot(avianSubBoundary,aes(x=V1,y=V2))+geom_polygon(alpha=0.3)+ theme_bw() + geom_point(data=avianSub,aes(x=lon,y=lat))+ labs(x="",y="",title="Paulet (Antarctic Fur Seals)")
grid.arrange(p1, p2, ncol = 2)
```
...
```{r, fig.width=17,echo=FALSE,message=FALSE,warning=FALSE}
x=runif(300);y=runif(300);xypoints=data.frame(x=x,y=y)
p1=ggplot(xypoints, aes(x, y)) + geom_point()+ theme_bw()
env=envelope(verbose=F,ppp(x=x,y=y,c(0,1),c(0,1)),fun=Kest,nsim=99)
df=data.frame('r'=env$r,'obs'=env$obs,'lo'=env$lo,'hi'=env$hi,'theo'=env$theo)
p2=ggplot(df)+geom_line(aes(x=r, y=obs,color='Observed'))+ geom_line(aes(x=r, y=theo,color='Theoretical'))+geom_ribbon(aes(x=r,ymin=lo,ymax=hi,alpha='Envelope'))+theme_bw()+ guides(color=guide_legend(title=NULL))+ guides(alpha=guide_legend(title=NULL))+ theme(legend.position=c(0.2,0.70))+labs(x="r",y="K")

x2=runif(100);y2=runif(100)+1;xypoints=data.frame(x=c(x,x2),y=c(y,y2),Intensity=c(rep('300',300),rep('100',100)))
p3=ggplot(xypoints, aes(x, y,color=Intensity)) + geom_point()+ theme_bw()+guides(colour=FALSE) 
env=envelope(verbose=F,ppp(x=c(x,x2),y=c(y,y2),c(0,1),c(0,2)),fun=Kest,nsim=99)
df=data.frame('r'=env$r,'obs'=env$obs,'lo'=env$lo,'hi'=env$hi,'theo'=env$theo)
p4=ggplot(df)+geom_line(aes(x=r, y=obs,color='Observed'))+ geom_line(aes(x=r, y=theo,color='Theoretical'))+geom_ribbon(aes(x=r,ymin=lo,ymax=hi,alpha='Envelope'))+theme_bw()+ guides(color=guide_legend(title=NULL))+ guides(alpha=guide_legend(title=NULL))+ theme(legend.position=c(0.2,0.70))+labs(x="r",y="K")
xypoints=data.frame(x=x,y=y)
p5=ggplot(xypoints, aes(x, y)) + geom_point() + ylim(0,2)+theme_bw()
env=envelope(verbose=F,ppp(x=x,y=y,c(0,1),c(0,2)),fun=Kest,nsim=99)
df=data.frame('r'=env$r,'obs'=env$obs,'lo'=env$lo,'hi'=env$hi,'theo'=env$theo)
p6=ggplot(df)+geom_line(aes(x=r, y=obs,color='Observed'))+ geom_line(aes(x=r, y=theo,color='Theoretical'))+geom_ribbon(aes(x=r,ymin=lo,ymax=hi,alpha='Envelope'))+theme_bw()+ guides(color=guide_legend(title=NULL))+ guides(alpha=guide_legend(title=NULL))+ theme(legend.position=c(0.2,0.70))+labs(x="r",y="K")
grid.arrange(p1, p5,p3,p2,p6,p4, ncol = 3)
```
...
p1=ggplot(avianSubBoundary,aes(x=V1,y=V2))+geom_polygon(alpha=0.3)+ theme_bw() + geom_point(data=avianSub,aes(x=lon,y=lat))+ labs(x="",y="")

df1=data.frame('r'=env.K$r,'obs'=env.K$obs,'lo'=env.K$lo,'hi'=env.K$hi,'theo'=env.K$theo)
p2=ggplot(df1)+geom_line(aes(x=r, y=obs,color='Observed'))+ geom_line(aes(x=r, y=theo,color='Theoretical'))+geom_ribbon(aes(x=r,ymin=lo,ymax=hi,alpha='Envelope'))+theme_bw()+ guides(color=guide_legend(title=NULL))+ guides(alpha=guide_legend(title=NULL))+ theme(legend.position=c(0.2,0.70))+ labs(x="r (meters)",y="K")

df2=data.frame('r'=env.G$r,'obs'=env.G$obs,'lo'=env.G$lo,'hi'=env.G$hi,'theo'=env.G$theo)
p3=ggplot(df2)+geom_line(aes(x=r, y=obs,color='Observed'))+ geom_line(aes(x=r, y=theo,color='Theoretical'))+geom_ribbon(aes(x=r,ymin=lo,ymax=hi,alpha='Envelope'))+theme_bw()+ guides(color=guide_legend(title=NULL))+ guides(alpha=guide_legend(title=NULL))+ theme(legend.position=c(0.7,0.20))+ labs(x="r (meters)",y="G")
...
```{r,echo=FALSE,fig.width=15}
p1=ggplot(pauletSubBoundary,aes(x=V1,y=V2))+geom_polygon(alpha=0.3)+ theme_bw() + geom_point(data=pauletSub,aes(x=lon,y=lat))+ labs(x="",y="")

df=data.frame('r'=env.K$r,'obs'=env.K$obs,'lo'=env.K$lo,'hi'=env.K$hi,'theo'=env.K$theo)
p2=ggplot(df)+geom_line(aes(x=r, y=obs,color='Observed'))+ geom_line(aes(x=r, y=theo,color='Theoretical'))+geom_ribbon(aes(x=r,ymin=lo,ymax=hi,alpha='Envelope'))+theme_bw()+ guides(color=guide_legend(title=NULL))+ guides(alpha=guide_legend(title=NULL))+ theme(legend.position=c(0.2,0.70))+ labs(x="r (meters)",y="K")

df=data.frame('r'=env.G$r,'obs'=env.G$obs,'lo'=env.G$lo,'hi'=env.G$hi,'theo'=env.G$theo)
p3=ggplot(df)+geom_line(aes(x=r, y=obs,color='Observed'))+ geom_line(aes(x=r, y=theo,color='Theoretical'))+geom_ribbon(aes(x=r,ymin=lo,ymax=hi,alpha='Envelope'))+theme_bw()+ guides(color=guide_legend(title=NULL))+ guides(alpha=guide_legend(title=NULL))+ theme(legend.position=c(0.7,0.20))+ labs(x="r (meters)",y="G")
grid.arrange(p1, p2,p3, ncol = 3)
```
...
```{r,fig.width=17,message=FALSE}
nearest.distances <- data.frame(
  Island=c(rep("Avian", avian.ppp$n), rep("Paulet", paulet.ppp$n)),
...
ggplot(nearest.distances, aes(x=first)) + geom_histogram() +
  facet_grid(Island~.)
p1=ggplot(nearest.distances, aes(x=first, y=second, color=Island)) + geom_density2d() + labs(y="Second NN Distance",x="First NN Distance")
p2=ggplot(nearest.distances, aes(x=first, y=third, color=Island)) + geom_density2d() + labs(y="Third NN Distance",x="First NN Distance")
p3=ggplot(nearest.distances, aes(x=second, y=third, color=Island)) + geom_density2d() + labs(y="Second NN Distance", x="Third NN Distance")
grid.arrange(p1, p2,p3, ncol = 3)
```
<br/>
Assigned Readings:
...
{ #open fori
 ...
 if ((length(temp$lengths)==2)&(temp$values[1]==1))
  { #open if
    ...
    temp$values[2]<-1
    temp$values[3]<-0
    matrix.ran<-cbind(matrix.ran,c(rep(0,times=shift),rep(1,times=range.length),rep(0,times=c)))
  } #close if
  } #close forj
...
permute_range <- function(data){ #open function
  ...
  for(i in 1:length(rangeSize)){ #open for
...
#least-squares regression line and have a
#data constraint envelope, based on sample size
...
plot(invasives$value~value,data=data.graph,ylab="Invasive Species Richness",xlab="Native Species Richness")
...
print(s.O)
```
The R squared value is `r (s.O<-lm.r$coefficients[2])` (not a good fit at all); maybe another function will have a better fit.  Further exploration could involve using a different model and comparing the new test statistic to a null distribution.  Yet, since we are mimicking Fridley et al. (2004), we will still use the null distribution of the correlation coefficient and s statistics.
...
for (i in 1:500)
  {
  new.data.Origin<-sample(species$Origin, size=length(species$Origin),replace=F)
...
```{r}
#to get the original dataset values, the r and s values code is re-run from the original dataset (since r and s object names were used later) 
